<template>
  ...
  <view class="custom-header">
        <view class="left-area" @click="goBack" role="button" aria-label="返回上一页">
          <image class="back-icon" src="/static/iconOrig/backButton.png" mode="aspectFit"></image>
        </view>
        <view class="title-area">
          <text class="header-title">志愿者家庭</text>
        </view>
...
const handleJoinFamily = async () => {
  ...
  try {
    ...
    uni.showToast({
      title: error.message || '加入失败，请稍后重试',
      icon: 'none'
    });
  } finally {
    uni.hideLoading();
