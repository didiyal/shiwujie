...
function openMapByAndroid(origin, destination,mode) {
	...
	var bdapp = "baidumap://map/direction?destination=name:"+destinationName+"|latlng:"+destination.latitude+","+destination.longitude+"&coord_type=wgs84&mode="+bdMode+"&src=uniapp";
	if(origin){
		bdapp = "baidumap://map/direction?origin=name:"+originName+"|latlng:"+origin.latitude+","+origin.longitude+"&destination=name:"+destinationName+"|latlng:"+destination.latitude+","+destination.longitude+"&coord_type=wgs84&mode="+bdMode+"&src=uniapp";
	}
...
function navigationByAndroid(destination){
	let destinationName = destination.name?destination.name:"终点";
	var bdapp = "baidumap://map/navi?location="+destination.latitude+","+destination.longitude+"&query="+destinationName+"&coord_type=wgs84&src=uniapp";
	var bdappDown = "http://map.baidu.com/zt/qudao/newfengchao/1012337a/html/slide.html"
...
function openMapByIos(origin, destination,mode) {
	...
	let destinationName = destination.name?destination.name:"终点";
	var bdMode = "driving";
	var bdapp = "baidumap://map/direction?origin=我的位置&destination=name:"+destinationName+"|latlng:"+destination.latitude+","+destination.longitude+"&coord_type=wgs84&mode="+bdMode+"&src=uniapp";
	if(origin){
		bdapp = "baidumap://map/direction?origin=name:"+originName+"|latlng:"+origin.latitude+","+origin.longitude+"&destination=name:"+destinationName+"|latlng:"+destination.latitude+","+destination.longitude+"&coord_type=wgs84&mode="+bdMode+"&src=uniapp";
	}
...
function navigationByIos(destination){
	let destinationName = destination.name?destination.name:"终点";
	var bdapp = "baidumap://map/navi?location="+destination.latitude+","+destination.longitude+"&coord_type=wgs84&query="+destinationName+"&src=uniapp";
	var bdappDown = "https://apps.apple.com/cn/app/id452186370";
...
export default {
	/* 路程规划 */
	routePlan(options,type="wgs84") {
		let _origin = options.origin,_destination = options.destination,_mode = options.mode?options.mode:"walk";
		...
		// #ifndef MP
			switch(uni.getSystemInfoSync().platform){
				case 'android':
					...
					this.directMapLaunch(_origin, _destination, _mode, 'android');
					break;
				case 'ios':
					console.log('运行iOS上')
					this.directMapLaunch(_origin, _destination, _mode, 'ios');
					break;
				default:
	...
	directMapLaunch(origin, destination, mode, platform) {
		let destinationName = destination.name ? destination.name : "终点";
		let originName = origin && origin.name ? origin.name : "起点";
		...
		plus.nativeUI.confirm(
			"请点击右下角开启导航",
			(e) => {
				if (e.index === 0) { // 用户点击了确定
					// 确定最适合的地图应用
					let targetUrl = this.determineMapUrl(navLinks, platform);
					// 打开地图应用
					this.launchMapApp(targetUrl);
				}
			},
			{
				title: "导航到 " + destinationName,
				buttons: ["确认", "取消"],
	...
	determineMapUrl(navLinks, platform) {
		// #ifdef APP-PLUS
		if (platform === 'android') {
			// 优先级：高德 > 百度 > 腾讯 > 默认浏览器
			if (isGaode) {
				return navLinks.gaode;
			} else if (isBaidu) {
				return navLinks.baidu;
			} else if (isQQ) {
				return navLinks.qq;
			} else {
	...
	launchMapApp(url) {
		...
		plus.runtime.openURL(encodeURI(url + '&time=' + time));
	}
}