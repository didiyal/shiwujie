...
android {
    ...
    defaultConfig {
        ...
        targetSdk = 35
        versionCode = 20
        versionName = "3.1.15"  // 3.1.15：WS 心跳 30s→15s + 心跳发送失败当场重连（断线感知减半）   // 3.1.14：志愿者等待窗不再自动复活（START_NOT_STICKY）；盲人连点防抖真正生效   // 3.1.13：身份/实名认证全部下线（双端不再弹窗不再展示）；志愿者端退后台不再播报   // 3.1.12：进入软件不再弹残疾证身份校验   // 3.1.11：紧急求助 60 秒无家属接通自动取消并播报；家属端收回弹窗时语音播报；接听落败播报   // 3.1.10：AI 任务进行中禁点语音/拍照；视频通话退后台不播提示   // 3.1.9：息屏保活（WakeLock）+ 拍照播报缩至 60 字   // 3.1.8：WS 未连接时禁止发起求助（防志愿者单侧干等）   // 3.1.7：挂断后通话标志复位（onResume）+ 服务端 type=5 对端通知   // 3.1.6：紧急求助状态残留自愈（退出竞态/杀进程不再卡死）   // 3.1.5：匹配失败 TTS 播报 + 服务端残留等待记录自动过期   // 3.1.4：WS 心跳安全网重连 + 重连后业务闸门复位   // 3.1.3：强更链路根修（DownloadManager URI + 缓存中转安装）   // 3.1.1：强更防旧缓存/循环（时间戳文件名+安装前版本校验）
