...
### ai（AI 对话 / SSE / 工具路由 / 记忆）`advisor/ agent/ app/ chatmemory/ controller/ service/ mapper/ tools/ utils/ constants/ config/` —— **已弃用·待删除（AI 重写设计敲定·实现待 Phase 5）**

> **整个 ai 子包（`advisor/`/`agent/`/`app/`/`chatmemory/`/`controller/`/`service/`/`mapper/`/`tools/`/`utils/`/`constants/`/`config/`）将删除**，替换为独立的 Python 自建 ReAct loop 服务（polyglot 双进程）。设计全貌见 [architecture/ai-rewrite.md](../../../docs/architecture/ai-rewrite.md)，Python 服务实现见 [shiwujie-ai/docs/](../../../shiwujie-ai/docs/)。重写后 bootstrap 角色变化：**从「自带 AI 实现」变为「网关 + MCP server」** —— Java 保留 WS 终结点 / JWT 鉴权 / user/call/community 业务真相源，并**新增 MCP server**（streamable HTTP，暴露业务 4 工具 `join_family`/`leave_family`/`family_info`/`update_profile` + 信令 4 工具 `launch_navigation`(5006)/`request_video_help`(5002)/`request_emergency_help`(5003)/`open_app`(5004)）供 Python 调用。**`agent/` 下的 MyManus（自研 ReAct 雏形）冻结保留非删** —— 作 Java-graph 备选 B-prime 的回退起跑线（见 [known-issues.md](../known-issues.md) ai 节归档说明）。下列现状描述保留作「被替换对象」参考；缺陷明细见 [known-issues.md](../known-issues.md) ai 节。
