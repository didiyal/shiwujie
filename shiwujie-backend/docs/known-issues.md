...
1. **Controller 层直接注入跨模块 Service（架构瑕疵，v3.0.0 起改本地）**：`CommunitymanagerController` 在 Controller 内注入 `InnerVolunteerService` 并直调（`deleteCommunityManager`），跨模块 Inner 调用应封装在 Service 层。v2.1.0 为 `@DubboReference`，v3.0.0 阶段2.2（`199e6f3`）改 `@Resource` 同进程注入——瑕疵仍在（Controller 直调 Inner），但已非 Dubbo。
