...
> v3.0.0 AI 重写（设计敲定·实现待 Phase 5）：整个 Java AI 模块将弃用并删除（`app/`/`agent/`/`tools/`/`advisor/`/`chatmemory/` 子包 + `AiConfig`/`AiConstants`/`ChatServiceImpl`/`ChatController` + 依赖 `spring-ai-alibaba-starter-dashscope`/`spring-ai-openai`/`kryo`/`jsoup` 若仅 AI 用），替换为 Python 自建 ReAct loop 服务（见 [architecture/ai-rewrite.md](../../docs/architecture/ai-rewrite.md) 与 [shiwujie-ai/docs/](../../shiwujie-ai/docs/)）。下列 ai 缺陷随模块删除消灭。**MyManus（自研 ReAct 雏形）冻结保留非删** —— 作 Java-graph 备选 B-prime 的回退起跑线。AiLogs 图片 offload 去留待 Phase 5。
>
