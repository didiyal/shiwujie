...
12. ✅ ~~AiWsRelayService HttpClient 默认 HTTP/2 致 Python /ai/turn 422~~（2026-07-25 修复，`6cbd0d8`）：`HttpClient.newBuilder()` 未指定 version → Java 默认 `HTTP_2`，对仅支持 HTTP/1.1 的 uvicorn 先发 h2c upgrade（python 日志 `Unsupported upgrade request` / `Invalid HTTP request received`），被拒后 fallback 时 **POST body 未正确重发** → Python 收到空 body → FastAPI 422（field required）。图片 turn（~770KB base64 body）必中、文本 turn 偶发。**修复**：显式 `.version(HttpClient.Version.HTTP_1_1)`，直接 HTTP/1.1 POST，body 正确送达。部署真机图片 AI-turn 时暴露——App→Java→Python 链路通，独卡此 422；python 容器内 `urllib` 直发合法 body 返 200 证伪了 schema 嫌疑，坐实是 Java 传输层。
