...
1. ✅ ~~**ai 默认用户兜底（生产后门）**~~（2026-09-12 修复，AI 链路鉴权并入业务拦截器）：原 `AiLoginCheckInterceptor` 无 Authorization 时注入 blindId=1 / phone=19872250169，任何人可白嫖 AI。**修复**：AI 专用拦截器 + `AiWebConfig` 整体删除，`/api/ai/**` 并入业务 `LoginCheckInterceptor`（无 token 即 `NOT_LOGIN` 拒绝，与 user/call/community 行为一致）；AI 侧身份取值从 `loginBlind` 实体属性切到 `loginBlindId`（`LoginUtils.getLoginBlindId()` 无参版），志愿者 token 访问 AI 返 `NO_AUTH`。副作用即收益：匿名调用者共享 `chat:memory:1` 上下文的污染面随之消失。原风险见 [architecture/auth.md](../../docs/architecture/auth.md) 风险 #6。
...
3. ✅ **续期 key 拼接 bug（已修复 2026-07-10）**：`renewKey`/`expire` 曾拼的 key **少了 `-blind-`/`-volunteer-` 段**（`LoginCheckInterceptor` 续期、`deleteBlind`/`deleteVolunteer` 删 token 均漏前缀；登录存/注销删/拦截器读则正确）→ 滑动会话静默失效，活跃用户 90 天后被踢；删用户旧 token 残留。**修复**：user/call/community 三份拦截器提取共享 `redisKey`（读/续期共用）杜绝拼接分叉，续期对齐登录 90 天；`BlindController:143`/`VolunteerServiceImpl:410` 删 token 补回前缀。ai 模块同型 bug（读 `-blind-{id}` 续期写 `-{id}`，续期永不生效）随 2026-09-12 AI 链路并入业务拦截器一并消失。原风险见 [architecture/auth.md](../../docs/architecture/auth.md) 风险 #1。
4. **JWT 过期校验被关闭**：`validateToken(..., true)` 第三参 `ignoreExp=true`，JWT 自身 exp 永不生效，过期完全依赖 Redis TTL。Redis 故障/误写则 JWT 形同永久有效。**注（2026-09-12）**：业务拦截器与（原）AI 拦截器均传 `true`——这是滑动会话（90 天 Redis TTL 管控生命周期）的统一设计而非 AI 特有缺陷；收紧与否（改 `false` + 登录发长 exp）归安全加固决策。见 [architecture/auth.md](../../docs/architecture/auth.md) 风险 #2。
...
12. ✅ ~~图片识别 + provisioned key 不通（2026-07-25 回退带来）~~（2026-09-10 实测推翻）：AI 回退到 `58184dc` 后 LLM key 换百炼 provisioned MaaS（OpenAI 兼容，同实例 `llm-8oompsig0r5hox8l`）。原判断「provisioned key 只认 MaaS compatible-mode，图像路径 `AiConfig.qwenImage`（`DashScopeChatModel` 原生端点）预期不通」——**实测不成立**：该 key 是百炼平台级 key（公开/provisioned 两端点均认证通过），图像路径经 `AiSmokeTest` 端到端验通（`qwen3-vl-flash` 正确识别测试图）。当时不通的真实根因是**账户欠费**（公开端点报 `Arrearage`、provisioned 实例报 `AccessDenied.Unpurchased`，充值后全恢复）。顺带修正：`AiSmokeTest` 默认 key/base-url 原与 application.yml 脱节（残留 2026-07-12 止血期普通 key + 公开端点），已同步为 provisioned 默认值。运维教训：provisioned 实例报 `Unpurchased` 未必是模型未部署，先查账户余额。
