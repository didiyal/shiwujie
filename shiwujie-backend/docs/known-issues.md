...
12. **图片识别 + provisioned key 不通（2026-07-25 回退带来）**：AI 回退到 `58184dc` 后 LLM key 换百炼 provisioned MaaS（OpenAI 兼容，同实例 `llm-8oompsig0r5hox8l`）。文本路径 `AiConfig.qwenText` 走 `OpenAiChatModel` 直连 provisioned compatible-mode 端点（已本地端到端验通）；但**图像路径 `AiConfig.qwenImage` 仍用 `DashScopeChatModel`（原生 `dashscope.aliyuncs.com` 端点）**，provisioned key 只认 MaaS compatible-mode → 图片识别（`/api/ai/ai/doChatByImage`）预期不通。bean 懒调用，应用能启动，仅调图片识别时报错；文本对话不受影响。后续修法：把 `qwenImage` 也改 `OpenAiChatModel` 接 provisioned（`qwen3-vl-flash` 多模态）。
