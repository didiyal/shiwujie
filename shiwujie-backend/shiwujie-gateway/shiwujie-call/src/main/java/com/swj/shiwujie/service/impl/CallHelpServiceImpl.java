...
public class CallHelpServiceImpl extends ServiceImpl<CallHelpMapper, CallHelp>
     ...
     */
    @Override
    public CallHelpVO getCallHelpBlindChannel(Long currentUserId) {
        User blindUser = innerUserService.getById(currentUserId);
        //1. 生成channel与uid
        ...
        innerUserService.updateById(blindUser);
        //4. 返回结果
     ...
     */
    @Override
    public CallHelpVO joinCallHelpBlindChannel(CallHelpJoinRequest callHelpJoinRequest, Long currentUserId) {
        //拿到求助人信息
        User helpUser = innerUserService.getById(currentUserId);
        ...
        User assistedBlind = innerUserService.getByChannel(channel);
        ...
        innerUserService.updateById(helpUser);
        innerUserService.updateById(assistedBlind);
        //3. 返回uid与channel
        return new CallHelpVO(channel,uid);
     ...
     */
    @Override
    public Boolean leaveCallHelpByBlind(Long currentUserId) {
        //拿到用户信息
        User blindUser = innerUserService.getById(currentUserId);
        //拿到callHelp的信息
        ...
        //2. 求助帮助结束挂断
            ...
            //   2. 修改自己与求助人的user表内容
            //修改盲人与志愿者的信息
            innerUserService.updateCallUsersInformation(callHelp.getChannel());
        }
     ...
     */
    @Override
    public Boolean leaveCallHelpByHelpUser(Long currentUserId) {
        //拿到用户信息
        User helpUser = innerUserService.getById(currentUserId);
        //拿到callHelp的信息
        QueryWrapper<CallHelp> callHelpQueryWrapper = new QueryWrapper<>();
    ...
    public CallHelp createNewCallHelpContent(User blindUser, String channel, String uid) {
        ...
        if (!innerFamilyService.familyUsersVerify(familyId)) {
