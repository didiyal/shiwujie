...
public class NewApp {
    ...
    @Resource
    private NewAppTools newAppTools;
    ...
    @Resource
    private NewAppToolChooseApp newAppToolChooseApp;
    ...
    private final String systemPromptEnd = """
            ...
            4. 若工具执行失败或缺少必要信息，请友善地说明原因，并引导用户提供所需内容,若是系统错误,则告诉用户是网络问题。
               例如：若跳转软件失败提示“缺少内容”，可回复：“跳转软件需要您提供具体的软件名字，能告诉我吗？”
            5. 注意上下文信息，冲突信息已最新的一条为准
            
            可调用的工具包括：
            1. 核心业务工具：处理软件，家庭、社区、活动、求助帖等操作。
            2. 网络搜索工具：用于联网查询信息。 
            3. 无需工具调用：问题已可直接回答。
            4. 需要更多信息：需向用户询问详情。
    ...
    public NewApp(OpenAiChatModel chatModel, EasyProblemAppRedisChatMemory easyProblemAppRedisChatMemory) {
    ...
    public Flux<String> doChatWithTextSSE(String text, Long blindId) {
        return Flux.defer(() -> {
            try {
                ...
                String problem = "你现在的语气是" + tone + "问题是" + text;
                String aiResponse = newAppToolChooseApp.doChatWithTextStart(problem, blindId);
                ...
                if (toolCallRequest != null) {
                    switch (toolCallRequest.getType()) {
                        case -1:// 执行业务工具调用
                            ...
                            content = newAppTools.questionChoose(toolCallRequest.getData());
                            break;
                        case -2:// 网页搜索
