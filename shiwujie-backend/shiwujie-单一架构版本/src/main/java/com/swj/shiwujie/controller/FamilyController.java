...
@CrossOrigin
@Slf4j
//@RequestMapping("/shiwujie/family")
public class FamilyController {
    ...
    public BaseResponse<FamilyVO> createFamily(String familyName, HttpServletRequest request) {
        //1. **从用户公共类中获取操作用户的id**
        if (userService.getCurrentUserId(request) == null) {
        ...
        FamilyVO familyVO = familyService.addFamily(familyName,userService.getCurrentUserId(request));
    ...
    public BaseResponse<FamilyVO> getFamilyByAccount(String familyAccount, HttpServletRequest request) {
        ...
        FamilyVO familyVO = familyService.getFamilyByAccount(familyAccount,userService.getCurrentUserId(request));
        //6. 将数据返回给前端(可以封装一个返回类)
        return ResultUtils.success(familyVO);
    }
    ...
    public BaseResponse<Boolean> deleteFamily(Long id, HttpServletRequest request) {
        //1. **从用户公共类中获取操作用户的id**
        Long loginUserId = userService.getCurrentUserId(request);
        if (loginUserId == null) {
            throw new BusinessException(ErrorCode.NOT_LOGIN, "未登录");
        }
    ...
    public BaseResponse<FamilyVO> joinFamily(String familyAccount, HttpServletRequest request){
        ...
        FamilyVO familyVO = familyService.joinFamilyBuAccount(familyAccount,userService.getCurrentUserId(request));
