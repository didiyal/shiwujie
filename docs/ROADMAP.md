...
> 大方向清单。`## 待实现`（当前工作版本 v3.0.0 单体化，v2.0.0 已于 2026-07-11 封版）→ `## 已完成`（按版本里程碑，**均为开发后抽取补档**——历史开发完成后再整理的文档，非预先规划）。bug / 重构 / 实现细节下沉 [CHANGELOG.md](CHANGELOG.md) + 各子项目 `known-issues.md`，本文件只列大方向。
...
> v1.0 / v2.0.0 均为开发完成后**抽取补档**的整理性文档（v2.0.0 已补精确封版日期 2026-07-11，阶段 0–9 日期仍为「约」）。明细见 [CHANGELOG.md](CHANGELOG.md) 历史时期；用户可见契约见 [product/](product/)。

### v2.0.0（二期微服务，阶段 0–9 累积 · 已封版 2026-07-11）

> 二期首个 semver 版本（一期 `v1.0` tag 在前），2026-07-11 封版（tag `v2.0.0`）。阶段 0–9（约 2025-06 ~ 2026-07）累积现状作起点，主体能力已建成。未完成的收尾项（🔴 安全加固 / 能力补全 / 工程化）整体平移至 v3.0.0（单体化）。
