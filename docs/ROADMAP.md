...
> 反思 v2.1.0 微服务对当前体量过度设计，去微服务、精简为单体（**已落地**，待 tag 封版）；并规划安全加固、能力补全与 AI 模块重写（polyglot：Java 业务单体 + Python 自建 ReAct loop 智能体）。单体化 + 安全加固 + 能力补全**已落地**；AI 重写**设计已敲定（Phase 1-4 梳理完成，见 [architecture/ai-rewrite.md](architecture/ai-rewrite.md)），实现待 Phase 5**——本段全部 `[ ]` 均未勾，诚实反映尚未落地。
...
[ ] 高德适配：App 集成高德 SDK
[ ] AI 重写-设计：Phase 1-4 梳理（功能/技术/方案/整合）已敲定，见 [architecture/ai-rewrite.md](architecture/ai-rewrite.md)（polyglot Java 业务单体 + Python 自建 ReAct loop 智能体两进程）
[ ] AI 重写-Python：自建 ReAct loop（langchain-core）+ 14 tool（6 Python-native + 8 Java-MCP）+ read_skill 元工具 + navigation skill + BM25 功能 KB + 两层记忆（短期手写 checkpoint / 长期偏好）
