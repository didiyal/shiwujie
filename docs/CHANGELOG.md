...
>
> git tag 现有 `v1.0`（一期·uniapp+单体，独立根提交）、`v2.0.0`（二期开发后初步稳定版，2025-11-12）、`v2.1.0`（二期微服务封版，2026-07-11）、`v3.0.0`（单体化封版，2026-09-12）。版本线：`v1.0`（一期）→ `v2.0.0`（二期初步稳定）→ `v2.1.0`（二期微服务封版）→ `v3.0.0`（单体化封版，2026-09-12）。阶段 0–9 是按能力域对提交的聚类，时间区间为代表性提交范围（部分阶段能力并行推进，区间有重叠），日期**不确定**（标「约」）；其累积现状作为 v2.1.0 的起点内容。打 tag 时把进行中版本的 `## vX.Y.Z（进行中，未发布）` 改为 `## vX.Y.Z - YYYY-MM-DD`（精确日期）。

---

## v3.0.0 - 2026-09-12

> 单体化改造版本（**2026-09-12 封版，tag `v3.0.0`**）。反思 v2.1.0 微服务对当前体量过度设计，去微服务、合并 user/call/community/ai 为单体（收敛为 model 契约层 + bootstrap 唯一 app 两模块、统一 Spring Boot 版本）。用户可见契约原则上不变（继承 v2.1.0），变更以工程架构为主。大方向见 [ROADMAP.md](ROADMAP.md)；交付任务见 [development/v3.0.0/task-breakdown.md](development/v3.0.0/task-breakdown.md)。封版范围补充（2026-09-12）：**LLM key 轮换与欠费恢复、Docker 编排默认仅 Java、App 相机闪退修复、AI 页四键改版与主页退场、句子级流式 TTS、强制更新、悬浮球改造、AI 链路鉴权并入业务拦截器、Web 官网 AI 化改版**——AI 重写（Python LangGraph）实现已回退（`ed65967`），成果休眠保留、延续至下一版本线。
...
- **删 Java AI 模块**：工作流式 prompt-as-router + 自研 ChatMemory + 半残留 RAG + qwen 止血整体替换；MyManus 自研 ReAct 骨架**原计划冻结保留**（342 行作 Java-graph B-prime 回退起跑线），**2026-07-20 撤销该决策、彻底删（见下 chunk-2b-6b）**。✅ chunk-2b / 2b-5（2026-07-19）已删活路径代码（`app/` + `controller/ChatController` + `service/ChatServiceImpl` + `tools/` + `chatmemory/` + `advisor/MyRagAdvisor` + AI 专用 common DTO + `AiSmokeTest` + `prompttemplate/*`，21 .java + 3 模板 + 1 测试，bootstrap 321 单测绿）；2b-5 侦查证 MyManus 仅依赖 `MyLoggerAdvisor`/`AiConfig`，不注入 tools/* 故可全删；`AiLoginCheckInterceptor`/`AiWebConfig` 已于 2b-6a（2026-07-20）删（dev 后门：无 Authorization 静默登录 blind id=1 / phone 19872250169；`/api/ai/**` 在 2b-5 删 `ChatController` 后已空集，拦截器拦空集，删除零功能影响；`ErrorCode` **保留**，53 个其它引用者）。WS phone 冒充（known-issues #7）的 ticket 鉴权留 chunk-2e 与 Android WS 改造同批。✅ 2b-4b（2026-07-20）紧急求助拆 `request_emergency_help_prepare`/`_confirm` 双工具落地（Java 侧闸 ②，红队 Q18）：新增 `EmergencyTokenStore`（Redis `ai:emerg:{token}` TTL 5min，绑 `(blind_id, issuing_turn)`，一次性消费）+ `BlindMcpContext.issuingTurn`/`McpTransportConfig` 加 `X-Issuing-Turn` header 提取（turn 概念来自 Python graph，经 MCP header 跨进程传，chunk-2c 真 MCP client 接）；`confirm` 拒同轮 token（结构闸 ②），跨轮通过才推 WS 5003；fail-closed（turn 未传→0→同轮永拒）；bootstrap 337 单测绿。gate ①（Python `parallel_tool_calls=False`）+ gate ③（App 显式确认面）留 Python/App 侧。✅ chunk-2b-6b（2026-07-20）撤销 MyManus 冻结保留决策 + 死代码闭环全删：`agent/`（BaseAgent/ReActAgent/ToolCallAgent/MyManus/model/AgentState）+ `advisor/MyLoggerAdvisor` + `config/AiConfig`（qwenText/qwenImage bean）+ `constants/AiConstants` + `utils/MessageSerializer`（kryo 孤儿）= 9 文件；bootstrap pom 清 5 依赖（`spring-ai-alibaba-starter-dashscope`/`spring-ai-openai`/`kryo`/`paho`/`jsoup`），父 pom 去 `spring-ai-alibaba.version` 属性 + `spring-ai-alibaba-bom`/`paho`/`kryo`/`jsoup` DM（留 `spring-ai-bom` 1.1.0 托管 MCP server）；`application.yml` 删 `spring.ai.dashscope`/`spring.ai.chat.client`/`search-api` 三块（留 `spring.ai.mcp` + `shiwujie.ai.python` 缝 A 配置）。撤销理由：零活路径引用（`AiWsRelayService` 的「ChatModel」仅 javadoc 提及、非注入）、B-prime 用 alibaba-graph 非自建 ReAct、红队 Q2 揭其「已弃用」真相——冻结沉没成本不如删净。bootstrap 337 单测绿（含 MyManus 全删后无编译漂移）。
- **两进程 Docker 部署**：Java 发布 8100:8100（公网），Python 不发布端口（仅内网，Java 经服务名调）；新增根级 `scripts/`（start/stop/logs/export/import/clear.sh）+ `docker/docker-compose.yml` + `config/.env`；非 docker 本地模式仍可。灰度 = 硬切换（后端镜像 + APK 同批发，SSE↔WS 不兼容须版本配对）。✅ chunk-2d（2026-07-20）Docker 物料落地：根级 `scripts/{start,stop,logs,export,import,clear}.sh`（bash，`-p shiwujie -f docker/docker-compose.yml`，`--build` 强制重建，`start.sh` 首跑自动 touch 空 `config/.env`）+ `docker/docker-compose.yml`（project `shiwujie`，两 service；`java` 发布 8100、`python` 仅 `expose 8500`；`extra_hosts host.docker.internal:host-gateway` + `environment` 覆盖 `MYSQL_HOST`/`REDIS_HOST=host.docker.internal` 连宿主 MySQL/Redis——yml 默认仍硬编码公网 IP 不变；`python` healthcheck `/health`，`java` 不 hard depends_on）+ 两多阶段 `Dockerfile`（backend：`maven:3.9-eclipse-temurin-21` install → `eclipse-temurin:21-jre`；ai：`python:3.12-slim` + `uv sync --frozen --no-dev` → 拷 `.venv`+`src`，`PYTHONPATH=/app/src`）+ `config/.env.example`（凭据全注释，走 committed 默认）+ 子项目 `scripts/start.sh`（本地非 docker 模式）+ `application.yml` `MYSQL_HOST`/`REDIS_HOST` 占位符化（默认硬编码公网 IP，Docker 覆盖 host.docker.internal）+ `shiwujie-ai/__main__.py` `SHIWUJIE_AI_HOST` env（默认 127.0.0.1，Docker 覆盖 0.0.0.0）。**端到端生产部署尚待**：✅ chunk-2c 核心（2026-07-20）真 qwen3.7-plus graph 内端到端验通（5 场景 + HTTP 流式 6/6 ✅，~15 LLM + 1 VLM 发，远低于 30-50 预算；`SHIWUJIE_AI_REAL=1` 开关切真模型、默认仍 FakeChatModel 保 122 测；详见 [shiwujie-ai known-issues](../shiwujie-ai/docs/known-issues.md) AI-4 + [e2e_real_go_nogo](../shiwujie-ai/reports/e2e_real_go_nogo.md)；真压缩/偏好/真 token delta 留下一轮 → ✅ chunk-2c补 wiring 全落（2026-07-20）：补A live-loop budget 接线（agent invoke 后熔断检查 + END）/ 补B 真 token delta（service 双流 `stream_mode=["messages","updates"]`，真 qwen 7-chunk token-by-token）/ 补C 真压缩（`memory/summarizer.py` LLM 摘要注入 `compress_messages`，异常兜假摘要）/ 补D 真偏好抽取（`memory/extractor.py` 信号闸 + structured_output + turn_end 后台 `create_task` fire，端到端实测入库）；140 测绿（+18 新），详见 [shiwujie-ai known-issues](../shiwujie-ai/docs/known-issues.md) AI-8）；剩 chunk-2e APK（App WS AI-turn 消费 + destination 字段 + 4-button）。✅ chunk-2e-2（2026-07-20）Android WS AI-turn 客户端落地：App 经既有 WS `/api/ws/call`（复用 `WebSocketManager` 监听机制不开新连接）发 requestType=100（裸 socketData 含 text+position）、收 110/111/112/113 四类帧路由到 UI + 讯飞 TTS（按句切喂避免逐 token 打断、turn_end 收尾 buffer 尾巴）；新增 `AiTurnManager`（turn 路由胶水层 + `setMatchingStatus(true)` 防 turn 中途 WS 重连丢 turn_end + 45s watchdog 兜底解锁麦克风防 relay 异常断连不发 113 卡死）+ `LocationHelper`（GPS→NETWORK→null + Geocoder 反查，后台线程，失败降级 null 不阻塞 turn）+ `SocketDataV0` 加 100/110-113 常量 + text/position/Position 内部类 + `toSendJson` 按 requestType 分支（仅 100 帧补 text/position，老帧 JSON 不变保向后兼容）；`AiFragment.sendAiRequest` HTTP+SSE 切 WS（保留 `AiChatManager` 作降级），复用既有 `showAiThinkingStatus`/`updateAiResponseStreaming`/`completeAiResponse`/`handleAiResponseError` 流式 UI 工具。`assembleDebug` 完整构建过 + `SocketDataV0` JSON 契约单测 7 例绿（守护上行帧结构 + position=null 字段缺省 + 老帧不变）；`AiTurnManager` 依赖 android.os.Handler/Looper（项目无 Mockito/Robolectric）靠端到端验证。剩 2e-1 destination 三端对齐 / 2e-3 4-button+相机 / 2e-4 紧急确认门 gate ③（token 独立帧 requestType=114，已定）/ 2e-5 WS ticket 鉴权。✅ chunk-2e-1（2026-07-20）destination 三端对齐 + 杀 volunteerPhone hack 落地：Java model 新增 `Destination{name,lat,lng,address}`（`@Data`，镜像 `Position`；v1 仅 name 必填，lat/lng/address 预留高德 SDK 精确定位 + Python 导航技能传坐标时扩签名填值）+ `SocketData` 加 `destination` 字段（hutool `JSONUtil.toJsonStr(socketVO)` 自动随 envelope 序列化，`CoordinationSocketHandler.noticeNavigation` 透传无需改）；`SignalMcpTools.launchNavigation` 杀旧 hack（`setVolunteerPhone(destination_name)` → 建 Destination 填 name 后 `setDestination`，方法签名 `destination_name`/`mode` 不动）；Android `SocketDataV0` 加 `Destination` 内部类（`@SerializedName` name/lat/lng/address，lat/lng 用 Double 装箱可空）+ 字段 + getter/setter（仅下行 5006 接收用，`toSendJson` 不动、上行不发 destination）；`AiFragment.handleNavigationRequest` 改读 `data.getDestination().getName()`（替代 `getVolunteerPhone()`，硬切无回退——Java+Android 同提交移动，3.10 灰度版本配对处理部署偏差）。后端 337 单测绿（`SignalMcpToolsTest.launchNavigation_pushes5006` 改断 `getDestination().getName()` + 断 `getVolunteerPhone()=null` 验 hack 已清）+ Android `assembleDebug` 过 + `SocketDataV0Test` 8 例绿（+2 destination 反序列化用例：5006 带 destination 正确解析 + 缺省得 null 不 NPE）。`openApp` 的 5004 volunteerPhone hack 不在本切片范围（3.8 只点名 destination），留后续清理。剩 2e-3 4-button+相机 / 2e-4 紧急确认门 gate ③（token 独立帧 requestType=114，已定）/ 2e-5 WS ticket 鉴权。✅ chunk-2e-4（2026-07-20）紧急确认门 gate ③ 落地（红队 Q18 第三道闸——盲人单声道无视觉冗余，须屏幕硬选择，agent 无法代点）：**后端** `AiWsTypes.OUT_EMERGENCY_TOKEN=114` + `InnerSocket.noticeEmergencyToken`/`InnerSocketImpl`/`CoordinationSocketHandler.noticeEmergencyToken`（镜像 noticeUrgentHelp 推 114 帧，socketData.text=token）+ `EmergencyTokenStore.consumeByApp(token, blindId)`（**不查轮次**——人工确认天然跨轮 + 用户知情，gate ③ 超越 gate ②；一次性消费 del key；错用户/未知 token 拒）+ `SignalMcpTools.prepare()` 签 token 后推 114 帧给 App（encode-不抛，推送失败 agent 仍持 token 可走 confirm() MCP 兜底）+ `UrgenthelpController POST /blind/confirm`（非-MCP HTTP 端点，agent 无此路径；消费通过 → 推 WS 5003）。**Android** `SocketDataV0.REQUEST_TYPE_EMERGENCY_TOKEN=114` + `AiTurnManager` 加 114 case → `onEmergencyToken(token)` 回调（**不进 turn 状态机**——prepare 在 turn 中途签发，仅透传给 UI，不碰 accumulator/watchdog）+ `ApiService.blindConfirmUrgenthelp`（`@POST /api/call/urgenthelp/blind/confirm` + `@Query("token")`）+ `EmergencyHelpManager.confirmEmergencyToken`（复用既有 urgenthelp HTTP 管线，独立 `EmergencyTokenConfirmCallback`，与通话生命周期正交：不建 Urgenthelp 记录、不动 isInEmergencyHelp）+ `AiFragment` 弹 `AlertDialog`（TTS 打断式播报提示语让盲人听到"是否通知家属" + `setCancelable(false)` 硬选择 + 防 agent 重复 prepare 堆叠 dismiss 旧弹 + onDestroy dismiss 防泄漏）。**additive 设计**：`confirm()` MCP（gate ②）保留不动，gate ③ 是平行 App 路径——**已知残差旁路**：agent 仍可走 cross-turn `confirm()` MCP 直接发 5003（不经 App 确认面），留用户硬化决策（封 confirm() MCP 仅 App 路径 / 保留双工具兜底），不擅自回退 2b-4b。后端 341 单测绿（337→341，+4 `consumeByApp` 用例 + `SignalMcpToolsTest.emergencyPrepare` 改断推 114 帧 requestType/blindPhone/text + never noticeUrgentHelp）+ Android `testDebugUnitTest` + `assembleDebug` 过。剩 2e-3 4-button+相机 / 2e-5 WS ticket 鉴权。✅ chunk-2e-5（2026-07-20）WS ticket 鉴权落地（堵 known-issues #7 phone 冒充，[auth.md](architecture/auth.md) L81-98 设计兑现）：现状 `@ServerEndpoint("/api/ws/call")` HTTP upgrade 零鉴权 + type=0 登录信客户端自报 phone → 任何人可冒充任意手机号建 WS 会话、收他人信令（5003 紧急/5006 导航）、以他人身份发 AI-turn。修复链路：**后端** 新增 `WsTicketStore`（镜像 `EmergencyTokenStore`，Redis `ai:ws:ticket:{ticket}` TTL 60s 一次性，值 JSON `{phone,role}`；`issue`/`consume` 返 `Optional<Bound>`，消费即 del 防重复建会话）+ `WsTicketController POST /api/call/ws/ticket`（路径落 `/api/call/**` 拦截器 scope 内复用 JWT，免改 WebConfig；盲人/志愿者共用，据 `loginBlindId`/`loginVolunteerId` 谁非空定 role）+ `SocketData` 加 `ticket` 字段（additive）+ `CoordinationSocketHandler.websocketLogin` **硬切**：信 ticket 不信自报 phone——`ticketStore().consume(ticket)` 通过才绑 session（`sessionMap`/`sessionPhoneMap` 用 ticket 绑定的真实 phone），无 ticket/失效**不绑 session、不回初始化成功**（客户端收不到信令即知失败、重连重取）；3.10 灰度版本配对，硬切正确。**Android** `SocketDataV0` 加 `ticket` 字段 + `createLoginMessage(phone,isVolunteer,ticket)` 签名 + `toSendJson` login 分支带 ticket（其余帧不带）+ `ApiService.fetchWsTicket`（`@POST /api/call/ws/ticket`）+ `WebSocketManager.connect` 拆 `fetchWsTicket`（鉴权 HTTP 换票，Retrofit enqueue 异步）→ `doConnect`（原 connect 主体，onOpen 发带 ticket 的 login）；ticket 取失败不建 WS、触发 `scheduleReconnect` 自愈（重连会再取票，因重连走 `connect()` 自动刷新 ticket）。**已知局限**：WS upgrade 本身仍零鉴权（jakarta.websocket 不经 DispatcherServlet，HandlerInterceptor 不作用），身份背书全走 ticket——升级后未带 ticket 的连接能建但绑不上 session、收发皆废，等同拒绝。design 说"顺手修 Redis 续期 key bug（known-issues #1）"——该 bug 在 JWT 拦截器、蔓延 scope，本切片不动、留单独修。后端 350 单测绿（341→350，+9 `WsTicketStoreTest`：issue 存 Redis + consume 一次性/未知/空/损坏/非 String + issue 异常传播）+ Android `testDebugUnitTest` + `assembleDebug` 过（`SocketDataV0Test` login 帧用例改断带 ticket）。剩 2e-3 4-button+相机。✅ chunk-2e-3（2026-07-20）4-button + 相机图片上行落地（task 3.8 APK 改造批末切片）：**设计抉择（响应骑 WS）**——图片 base64 达 MB 级超 WS 帧上限，故**上行走 HTTP multipart**；但 VLM 流式答复**复用文本 turn 的 WS 中继通路**推回（免新建 StreamingResponseBody 流式 HTTP 端点 + Android ndjson 解析重复），App 端 `AiTurnManager` 现成路由零改动复用。entry fork（design ② 拍照绕 loop 直连 VLM）**延后**——现状 agent 收默认提示调 `recognize_photo` 读 contextvar 图功能正确，确定性 fork 留能力补全批。**Python** `service/app.py` `/ai/turn` 已有 `image` 字段但未接线，补 `set_image_context(req.image)`（仿 `emergency.set_turn_context`，灌 vlm contextvar 供 `recognize_photo` 工具体读图——ToolNode 只传 args 不传 state 故走 contextvar）；None = 无图走 mock。**Java** `AiWsRelayService` 重构加 `submitImageRelay(session,blindId,text,imageDataUrl)`（提取 `submitInternal`/`beginTurn` 共用，`buildBody` 加 `image` 字段条件塞入，文本 turn 签名/行为不变）+ 新增 `AiImageTurnController POST /api/call/ai/image-turn`（multipart 收 `image`+可选 `text`，JWT 解 blindId+phone → `CoordinationSocketHandler.sessionMap.get(phone)` 查 WS session → 图转 base64 data URL → `submitImageRelay` 即返 200；路径落 `/api/call/**` 拦截器 scope 复用 JWT 免改 WebConfig；无 WS session 拒——图片 turn 无回执通道）+ `application.yml` 加 `spring.servlet.multipart.max-file-size: 10MB`（SB 默认 1MB 装不下手机拍照 JPEG 2-5MB）。**Android** `AiTurnManager` 加 `beginImageTurn()`（复用 sendTurn 前奏：清 accumulator + `setMatchingStatus(true)` 锁重连 + 挂 45s watchdog，**不发 WS 帧**——图片走 HTTP）+ `abortTurn(msg)`（上传失败等立即走 `listener.onError` + reset，复用 AiFragment 错误清理路径，不等 watchdog）+ 提取私有 `beginTurn()` 共用；`ApiService` 新增 `sendAiImageTurn`（`@POST /api/call/ai/image-turn` multipart，返 `BaseResponse<Void>` ack，`@Part image` + `@Part("text")`）；`ImageRecognitionManager` 加 `ImageTurnCallback`（onUploaded/onError）+ `sendImageTurn(File,text,cb)`（multipart 上传，仅 ack——流式响应骑 WS）；`AiFragment.sendImageRecognitionRequest` 重写（锁输入 + "正在思考"卡片 + `beginImageTurn` → HTTP 上传图+默认提示"请描述这张图片的主要内容" → 成功等 WS 帧、失败 `abortTurn`），与文本 turn `sendAiRequest` 共用同一套 WS 响应 UI+TTS（onDelta/onProgress/onTurnEnd）。布局 `fragment_ai_assistant.xml` 的 4-button（btn_back/btn_camera/btn_voice/btn_ai_assist）+ `CameraPreviewManager.takePhoto` 拍照管线**本就就位**，本切片仅换上行通道（老 SSE→新 HTTP+WS）。**死代码处置**：老 SSE 图片通道（`ImageRecognitionManager.sendImage`/`handleStreamingResponse` + AiFragment 老图片 listener + 智能播报系统 startSmartPlaybackSystem 等 6+ 方法）指向的 `/api/ai/ai/doChatByImage` 后端 2b-5 已删，但清理缠智能播报系统、整体删留独立重构切片——`ApiService.sendAiImageMessage` 标 `@Deprecated` 留编译（dormant 无调用方）。Python 140→142 测绿（+2 image wire-in：`/ai/turn` 带 image → 调 `set_image_context` 传同值 + 无 image 传 None；contextvar→recognize 链路见 test_vlm）+ 后端 350 单测绿（AiWsRelayServiceTest 仅测静态 parse/toFrame，submitRelay/buildBody 签名重构不破）+ Android `testDebugUnitTest` + `assembleDebug` 过。**chunk-2e APK 改造批（task 3.8）完结**（2e-1/2/3/4/5 五子切片全落）。剩：真机端到端验通（VLM real 需 `SHIWUJIE_VLM_FORCE_REAL=1`+key）、删老 SSE 图片死代码 + 智能播报系统（独立切片）。

✅ chunk-2f-1（2026-07-20）老 SSE AI 客户端栈全清（兑现 2e-3 deferred「删图片死代码 + 智能播报系统」，翻转 2e-2「AiChatManager 保留作降级」）：后端 SSE 端点 `/api/ai/ai/doChatByText`/`doChatByImage`/`/NewApp` 已于 chunk-2b-5（`35aca0d`，先于 2e-2）删除，App 侧老 SSE 客户端（`AiChatManager` 文本通道 + `ImageRecognitionManager.sendImage` 图片通道 + 二者共用的「智能播报系统」UI/TTS 状态机）全成调 404 的死代码——2e-2「AiChatManager 保留作降级」在 backend 已删的前提下是空头支票，本批撤销该保留决策、全清。**删**：`AiChatManager.java` 整文件（448 行，文本 SSE + `OnStreamingListener` + `handleStreamingResponse` 打字机）+ `ImageRecognitionManager` 瘦身为仅 `sendImageTurn`/`ImageTurnCallback`（删 `sendImage`/`OnStreamingListener`/`handleStreamingResponse`/executor/mainHandler/typingSpeed 等老 SSE 机械）+ `ApiService` 删 `sendAiTextMessage`/`sendAiImageMessage`（两废弃 SSE 端点）+ `AiFragment` 删两老 listener 块（`initAiChatManager` 整方法 + `initImageRecognitionManager` gut 为单行 `new`）+ 智能播报系统 11 方法（`startSmartPlaybackSystem`/`handleSmartPlayback`/`handleStreamingChunk`/`startSmartTimer`/`updatePlaybackContent`/`getCurrentPlaybackProgress`/`startSmartPlayback`/`parseResponseForTTS`/`startStreamingPlaybackUpdateTimer`/`completeSmartPlayback`/`resetSmartPlaybackState`）+ `StreamingState` enum + 11 个 handler/buffer/常量字段 + 4 个图片专属 UI（`showImageRecognitionStatus`/`updateImageRecognitionStreaming`/`completeImageRecognition`/`handleImageRecognitionError`，仅死 listener 调）+ lifecycle 清理点（preCleanup/onDestroy/safe-stop 的 smartTimer/streamingPlayback handler 清理 + `aiChatManager.destroy`/`stopStreaming` + `imageRecognitionManager.destroy`，后者随类 `destroy()` 一并删）。**留**：`resetAiResponseState`（live——`completeAiResponse`/`handleAiResponseError` 在 WS turn 路径调）+ WS UI 三件套（`updateAiResponseStreaming`/`completeAiResponse`/`handleAiResponseError`，`AiTurnManager` listener 路由）+ `sendImageTurn`/`ImageTurnCallback`（2e-3 新图片上行）+ `AiTurnManager` 全套 + `isReturningToHome` 字段（live，home-return 逻辑 set）。共删 ~636 行（AiFragment -636 / ApiService -27 / ImageRecognitionManager 重写瘦身 / AiChatManager 整删）。`assembleDebug` + `testDebugUnitTest` 过（`SocketDataV0Test` 等守护测绿）；后端零改（端点 2b-5 早删）、对外契约零变更。
