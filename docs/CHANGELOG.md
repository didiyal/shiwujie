...
> 迭代历程，按 git 开发阶段叙述（**无版本标签、无 semver 目录**）。每阶段四分类：新增 / 变更 / 修复 / 移除。提交 hash 可用 `git show <hash>` 溯源。
>
> 本文件**不创建任何 git tag**；阶段划分是按能力域对提交的聚类，时间区间为代表性提交范围（部分阶段能力并行推进，区间有重叠）。
...
### 移除
- **mqtt 硬件 IoT 通道**：因硬件成本问题取消，代码删除（**pom 残留 paho 依赖**，见 [`backend/ai.md`](backend/ai.md) 已知问题）。（git 历史可溯，代码已无 MqttClient 引用）
- **自研 ReAct Agent**：经大量测试（多种提示词、RAG 注入、Agent 调用），确定工具调用最佳方案为**代码实现工作流**，自研 ReAct 框架（`MyManus` 等）弃用、`@Component` 注释（保留代码未启用）。（`07459f6`，见 [`backend/ai.md`](backend/ai.md)）
- **RAG 知识库增强**：测试后效果不及工作流，移除接入（`MyRagAdvisor` Bean 半残留，未注入）。（`07459f6`）
- **语气切换功能**。（后续 `088b14f` 彻底删除）

---

## 阶段 6 · AI 能力跃升（2025-08-14 ~ 08-20）
...
## 阶段 5 · AI 模块从零到能用（2025-08-04 ~ 08-13）
...
- 工具调用、提示词设计。（`b65c7f8`）
- ai 模块两接口，上传图片识别，流式返回。（`a13a9dd` 2025-08-11）
- 基本工具非流式调用。（`e104ed4`）
- 工具调用流式输出（拆解优化）；AI 对话流式输出展示。（`dc4200b` / `3ff0e56` 2025-08-12）
...
## 阶段 3 · 视频通话与紧急求助（2025-07-21 ~ 08-13）
...
## 阶段 1 · 二期微服务脚手架（2025-07-21 ~ 08-01）
