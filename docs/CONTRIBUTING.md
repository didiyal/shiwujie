...
| 开发交付 | 各子项目 `docs/` + `docs/architecture/` | **技术实现** | 随代码就近 |

四层各司其职、互不重复：**ROADMAP 定方向 → CHANGELOG 记全部明细 → product 写用户能看见/能验收的契约 → development 写怎么实现**。

> 与母版的关键差异：本项目是**前后端 + 微服务多子项目**的工程，不是单体。故 development 层**不集中在一个 `docs/development/` 目录**，而是**随代码就近**放在各子项目 `docs/`（`shiwujie-backend/docs/`、`shiwujie-frontend/app/docs/`、`shiwujie-frontend/web/docs/`）+ 根 `docs/architecture/`（跨切面概览）。product / ROADMAP / CHANGELOG 仍**只集中在根 `docs/`** 一处——契约是全局的、实现是分散的。
...
Phase2/                              ← 仓库根（工作区恒设此层）
  ...
  docs/                              ← 平台层：外部可见（概览 + 用户契约 + 规范 + 方向/明细）
    ...
    README.md                        文档中心首页：当前基线 + 入口 + 历史时期指针
    ROADMAP.md                       大方向（历史时期 rolled-up + 待实现）
    CHANGELOG.md                     变更明细（历史时期 banner + 阶段 9..0；首 tag 后加 vX.Y.Z）
    product/
      current/                       当前工作基线（滚动，4 文件）
        product-overview.md
        functional-requirements.md
        acceptance-criteria.md
        changelog.md                 → 指向根 CHANGELOG
      vX.Y.Z/                        （首 tag 后才出现）版本快照，同构 4 文件
    architecture/                    跨切面概览（概念图/链路图/选型/分库设计/契约清单）
      overview.md / tech-stack.md / gateway-dubbo.md / auth.md / data-model.md
  shiwujie-backend/docs/             ← 后端 development 细化（内部·详细）
    README.md / modules/*.md / known-issues.md / deployment.md
  shiwujie-frontend/
    app/docs/                        ← Android development 细化
    web/docs/                        ← Web development 细化
```

- **product 每版 4 文件**：overview（是什么）/ functional-requirements（`FR-<MODULE>-<NN>`）/ acceptance-criteria（`AC-<MODULE>-<NN>`）/ changelog。
- **development 随代码就近**：每微服务一篇 `modules/<svc>.md`（定位/核心类/数据流/配置要点）+ 跨模块的 `known-issues.md`（缺陷/技术债）+ `deployment.md`（启动/部署坑）。
- **`product/current/` 是滚动工作基线**：始终反映当前主分支的累积现状；首 tag 后 `cp -r current vX.Y.Z/` 冻结快照，`current/` 继续向前滚。
...
- 首打 tag 后新增 `## vX.Y.Z - YYYY-MM-DD`（精确日期，来自 tag）。
...
- 四文件分工：overview = 项目是什么（用户视角：定位/三身份/覆盖范围/核心目标/非目标）；functional-requirements = 全部模块 `FR-<MODULE>-<NN>` 合并去重；acceptance-criteria = 全部 `AC-<MODULE>-<NN>` 合并去重；changelog = 指向根 CHANGELOG（历史时期）+ 未来各版本条目。
...
> architecture 与 modules 的边界：architecture 写**跨多个服务的切面**（一张图/一张表覆盖全局）；modules 写**单服务内部**。二者都属 development 层，允许技术细节。

## 四、内容边界对照表

| 内容 | product | development（modules/architecture/known-issues） |
|---|---|---|
...
- **本项目当前无 tag**：阶段 0–9 的历史**不回溯打 tag、不建 `product/阶段N/`**，统一并入 CHANGELOG 的「历史时期」banner（日期不确定标注）。`product/current/` 是这段历史的累积现状基线。
- **首 tag（计划 v1.0.0）后**：① `cp -r docs/product/current docs/product/v1.0.0/` 冻结快照；② 在根 `docs/development/v1.0.0/`（跨子项目）放 task-breakdown / testing-strategy / release-checklist 三件套；③ CHANGELOG 加 `## v1.0.0 - <tag 日期>`；④ ROADMAP 勾掉对应项。此后每 tag 同构。
- **状态标注**：进行中版本在 CHANGELOG / overview 标「进行中；已发布最新基线为 current」。
...
- **链接有效性**：`docs/README.md`、各 product/architecture 文件、子项目 `docs/README.md` 内相对链接可达（`git mv` 改路径后逐一修）。
- **目录完整**：`product/current/` 4 文件齐；`shiwujie-backend/docs/modules/` 6 篇齐（gateway/model-commonweb/user/call/community/ai）；frontend app/web 各有 docs。
...
- **代码改 → 文档同步**：任何行为/特性变更须更新 CHANGELOG（必要时报 ROADMAP、刷 `product/current/` 契约），勿留陈旧。
...
2. `docs/product/current/`——该子项目实现的契约（FR/AC/HTTP 路径/WebSocket 码/Dubbo 接口/表）。
3. 相关 `docs/architecture/*.md`——跨切面契约（路由图/调用图/鉴权链路/分库）。

> 子项目 `docs/` 仅 development 细化，**不含契约**。**别把子项目隔离成独立 workspace**（否则根文档不自动出现）；工作区恒 = 仓库根。

### 阶段回卷仪式（development 分散、契约集中 → 有同步缝，需回卷）

- **行为变更** → 即时进 CHANGELOG（轻量，小改顺手）。
- **阶段结束 / 打 tag 前 → 覆盖全项目的 agent 回卷**：① 刷 `product/current/` 契约（若有用户可见变更）；② 写该阶段 CHANGELOG 条目；③ ROADMAP 勾项；④ tag 时快照 `product/vX.Y.Z/` + 建 `development/vX.Y.Z/` 三件套。
