...
## AI 拦截器 dev 后门（重写必修删除项）

> 泛指「AI 拦截器 dev 后门」（类见 [backend known-issues](../../shiwujie-backend/docs/known-issues.md) #1、本篇风险 #6）：无 Authorization 时注入 blindId=1 / phone 固定值，生产未关闭则任何人可白嫖 AI。该后门是 AI 重写的**必修删除项**（重写后 AI 拦截器整体随 Java AI 模块弃用，鉴权统一回归 Java WS 网关 + ticket 机制）。两套编号各属局部、互相交叉引用，不发明第三个编号。
