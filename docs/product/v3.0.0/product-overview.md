...
- **用户可见契约**：原则上**保持不变**（API 路径 / WebSocket 信令 / FR / AC 继承 v2.1.0）。实际变更（如入口/端口收敛）随单体化设计滚动记录于本目录 [functional-requirements.md](functional-requirements.md) / [acceptance-criteria.md](acceptance-criteria.md) / [changelog.md](changelog.md)。
