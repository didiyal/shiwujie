...
> 本文件对应 `product/v2.1.0/`（二期·已封版 2026-07-11，tag `v2.1.0`）。v2.1.0 是二期微服务封版版本（一期 `v1.0`、二期初步稳定 `v2.0.0` 2025-11-12 在先）；其起点 = 阶段 0–9 的累积现状，完整四分类明细（新增 / 变更 / 修复 / 移除）在根 [CHANGELOG.md](../../CHANGELOG.md) 的「历史时期」banner 下（日期不确定）。当前工作版本已转 v3.0.0（见 [../current.md](../current.md)）。

## v2.1.0 - 2026-07-11

v2.1.0 = 二期微服务封版版本（v2.0.0 二期初步稳定之后），内容起点为阶段 0–9 累积现状。用户可见面的演进要点：
