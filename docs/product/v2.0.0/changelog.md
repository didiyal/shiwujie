...
> 本文件对应 `product/v2.0.0/`（二期·已封版 2026-07-11，tag `v2.0.0`）。v2.0.0 是**二期首个 semver 版本**（一期 `v1.0` tag 在前）；其起点 = 阶段 0–9 的累积现状，完整四分类明细（新增 / 变更 / 修复 / 移除）在根 [CHANGELOG.md](../../CHANGELOG.md) 的「历史时期」banner 下（日期不确定）。当前工作版本已转 v3.0.0（见 [../current.md](../current.md)）。
