# 功能需求（v2.0.0 · 已封版 2026-07-11）

> 全部模块的功能需求合并去重，编号 `FR-<MODULE>-<NN>`。`✅` 已实现 / `⚠` 部分（见备注）/ `❌` 已弃用·未启用·未实现。本文件属 `product/v2.0.0/`（已封版 2026-07-11）；技术实现（核心类/数据流/源码引用）见各子项目 `docs/modules/`，**不在此重复**。状态列如实反映当前契约边界（含试错移除能力）。
