...
- **强更版本链（三处必须同步）**：`app/build.gradle.kts` 的 versionCode/versionName ↔ `shiwujie-backend/.../application.yml` 的 `APP_VERSION_CODE/NAME`（env 可覆盖，当前 17 / 3.1.12）。客户端强更判定：本地 code < 服务端 code 即弹不可取消更新。测强更时客户端本地版本必须低于服务端，否则静默不弹（这是设计行为）。
