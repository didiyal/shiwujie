...
LLM 看到的 toolset 是**统一的 15 项**（14 工具 + read_skill），但来源分三类，LLM **不感知**来源差异（经 `MultiServerMCPClient.get_tools()` 把 MCP 工具并入 toolset，与 native StructuredTool 同形）：
...
**短期压缩策略**：超 token 阈值（~8–16k，Phase 5 按 qwen 实测定）把**最早一批** messages 压成滚动 summary；**recent-tail（约最近 10 条）永不压缩**——护导航中等需要近期状态连续的场景（用户刚说的终点、刚选的交通方式不能被压没）。崩溃 / 中途截止可从 checkpoint 恢复（含已压缩的 summary）。
...
Decision A（原生 function-calling 杀 2-call 税）**依赖 qwen FC 稳**。spike **前置**测本工具集 + 本 prompt 通过率（建议 ≥90%）。**无论 spike 结果都上两护栏**：① MCP 服务端 **strict JSON-schema 校验**（拒违例入参）；② **tool-name 白名单**（拒未注册名，堵幻觉名冒充 confirm——如模型编出 `confirm_emergency` 伪名）。详见 [known-issues](known-issues.md)「qwen FC 可靠性待 spike」。
