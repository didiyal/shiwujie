...
**2b-3b/4/5/6（待）**：业务工具真身（family_info/leave_family/join_family/update_profile → `InnerFamilyService`/`BlindService`，bind `BlindMcpContext.blindId`；update_profile 窄 DTO 硬卡 nickname/phone/gender，design ⑬ 硬修正 2，加单测断言无敏感字段 setter）+ strict schema（2b-3b 业务 / 2b-4 信令，⑬ 硬修正 3 两护栏 Java 侧收口）；信令 4 工具真身 + `SocketData` 加 `destination` 字段供 5006 载终点（2b-4）；删旧 Java AI 模块（`tools/app/*`、`tools/mytools/*`，MyManus 冻结保留，2b-5）；WS ticket 鉴权堵 phone 冒充 + 删 `AiLoginCheckInterceptor` dev 后门（2b-6）。whoami spike 工具随业务工具真身落地移除（生产 LLM 不应见）。
