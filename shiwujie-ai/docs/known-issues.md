...
**2b-4/5/6（待）**：strict schema（2b-4 信令，⑬ 硬修正 3 两护栏 Java 侧收口）；信令 4 工具真身（launch_navigation/request_video_help/request_emergency_help 拆 prepare/confirm/open_app → 进程内 `InnerSocket.noticeXxx` 推 WS 5006/5002/5003/5004）+ `SocketData` 加 `destination{name,lat,lng,address}` 字段供 5006 载终点 + 信令参数定稿（destination_name/lat/lng/mode/app，2b-4）；删旧 Java AI 模块（`tools/app/*`、`tools/mytools/*`，MyManus 冻结保留，2b-5）；WS ticket 鉴权堵 phone 冒充 + 删 `AiLoginCheckInterceptor` dev 后门（2b-6）。
