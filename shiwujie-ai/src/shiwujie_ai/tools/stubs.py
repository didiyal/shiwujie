"""工具桩结果（chunk-2a 工具体 stub 返回；C2a-4 升级为 mock-external-API / 真 VLM / 真 MCP）。

生产工具体在 chunk-2a 用本桩（让 graph 能跑通控制流），后续逐步换真实现：
- native（C2a-4）：recognize_photo→真 VLM / web_search→searchapi / get_weather→天气API / gaode_*→高德REST / search_kb→BM25。
- java_mcp（chunk-2b）：9 工具改由真 MCP server 加载（本地桩废弃）。
- read_skill（C2a-5）：改读真 SKILL.md。

桩内容「够用」——让多轮流程能继续（poi 桩带 lat/lng 供 launch；emergency prepare 桩带 token 供 confirm）。
spike FC runner 亦用本桩（经 re-export 在 spikes/fc/stubs.py），故此处为单一真值源，勿在他处重复。
"""

#: navigation 技能文档正文（read_skill("navigation") 的返回；真实 SKILL.md 在 C2a-5 由 loader 读）。
...
def stub_result(name: str, args: dict | None) -> str:
    """返回工具 name 的桩结果（chunk-2a graph 控制流用；未知工具给通用 ok）。
